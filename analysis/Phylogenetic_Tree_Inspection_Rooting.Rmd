...
knitr::opts_chunk$set(echo = TRUE, 
                      fig.align = "center",
                      # Always relevant to document directory
                      fig.path = "../figures/03_PhylogeneticTree") # send any figure output to this folder
```
# Goals
1. Load the fasttree unrooted tree
2. Add tree to phyloseq object
3. Visualuze and inspect tree with ggtree
...
# Load in the tree!
unrooted_tree <- read.tree("data/03_PhylogeneticTree/ASVs_unrooted.tree")
unrooted_tree
str(unrooted_tree)
```
...
# Merge Phyloseq
```{r merge-physeq}
stopifnot(ntaxa(raw_preprocessed_phyloseq) == ntaxa(unrooted_tree))

# Merge the tree with the phyloseq onject
unrooted_physeq <-
  merge_phyloseq(raw_preprocessed_phyloseq, unrooted_tree)
unrooted_physeq
```
...
# Zoom in on original tree (zoom button): Node 2537
...
save(list = c("unrooted_physeq_rm456","midroot_physeq_rm456"),
     file = "data/03_PhylogeneticTree/phytree_preprocessed_physeq.RData")
```
